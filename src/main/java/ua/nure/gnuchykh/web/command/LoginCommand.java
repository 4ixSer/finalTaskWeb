...
public class LoginCommand implements ActionCommand {
    ...
    public String execute(HttpServletRequest request) {
        ...
        if(user == null||LoginManager.loginIsFalse(user, pass)) {
            ...
            LOG.debug("ёзера с login "+user.getLogin()+"; password "+user.getPassword() +" не сушествует");
...
//            request.setAttribute("language", Locale.getDefault());
            ...
            LOG.debug("ќткрытие сесии дл€ "+ user.getType()+"; login= "+user.getLogin()+";\npassword= "+user.getPassword() +"; idSession= "+ session.getId());
      ...
      /* было     if (LoginLogic.checkLogin(login, pass)) {
            ...
            HttpSession session = request.getSession();
            System.err.println(" Login=> «апись в сесию значени€ " + ClientType.ADMINISTRATOR);
